        ...
        <div class="jumbotron">
            ...
            if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
                ...
                if ($Content !== false && strpos($Content, '\\') !== false) {
                    $macAddress = trim(substr($Content, strpos($Content, '\\')-20, 17));
                }
            } else {
                $output = shell_exec("ip addr show | grep 'link/ether'");
                if ($output) {
                    preg_match('/([a-fA-F0-9]{2}[:-]){5}[a-fA-F0-9]{2}/', $output, $matches);
                    $macAddress = $matches[0] ?? null;
                }
            }
            $macAddress = $macAddress ? $macAddress : 'MAC Address not found';

            // Get Hardware ID
            $hwId = null;
            if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
                $hwId = shell_exec('wmic csproduct get uuid');
                if ($hwId) {
                    $hwId = trim(str_replace('UUID', '', $hwId));
                }
            } elseif (strtoupper(PHP_OS) === 'DARWIN') {
                $hwId = shell_exec("system_profiler SPHardwareDataType | awk '/Hardware UUID/{print $3}'");
                if ($hwId) {
                    $hwId = trim($hwId);
                }
            } else {
                if (file_exists('/etc/machine-id')) {
                    ...
                    $hwId = shell_exec('dmidecode -s system-uuid');
                    if ($hwId) {
                        $hwId = trim($hwId);
                    }
                }
            }
            $hwId = $hwId ? $hwId : 'Hardware ID not found';
            ?>
            ...
            <div>
                <table class="table">
                    <tr>
                        ...
                        <td><strong><?php echo $macAddress; ?></strong></td>
                    </tr>
                    <tr>
                        <td><strong>Hardware ID</strong></td>
                        <td><strong><?php echo $hwId; ?></strong></td>
                    </tr>
                    <tr>
                        <td colspan="2">&nbsp;</td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
                            if ($checks['php']) {
                                echo "<i class='fa fa-ok text-success' aria-hidden='true'></i>";
                            } else {
                                echo "<i class='fa fa-remove text-danger' aria-hidden='true'></i>";
                ...
                <center>
                    ...
                    <p>
                        ...
                        echo "<br/><small style='font-size:13px'><a href='https://programmaticsurface.com/docs/getting-started/installing-pos/' target='_blank'>Installation Document</a></small>";
                        ?>
                    </p>
                </center>
