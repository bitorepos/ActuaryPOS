...
function projectModuleUrl(path) {
    var marker = '/project/';
    var pathname = window.location.pathname || '';
    var index = pathname.indexOf(marker);

    if (index !== -1) {
        return window.location.origin + pathname.substring(0, index).replace(/\/$/, '') + path;
    }
...
function showLoading(selector) {
    var $el = $(selector);
    if ($el.find('.project-loading-overlay').length === 0) {
        $el.css('position', 'relative').append(
            '<div class="project-loading-overlay" style="position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(255,255,255,0.7);z-index:10;display:flex;align-items:center;justify-content:center;">' +
            '<i class="fas fa-spinner fa-spin" style="font-size:24px;color:#3b82f6;"></i></div>'
        );
    }
}

function hideLoading(selector) {
    $(selector).find('.project-loading-overlay').remove();
}

function initializeProjectFormModal($modal) {
    var $container = $modal && $modal.length ? $modal : $('#project_model');
    var $form = $container.find('form#project_form');
    ...
    initializeProjectStepRows($form, $container);
}

function initializeProjectStepRows($form, $container) {
    var $table = $form.find('#project_steps_table');

    if (!$table.length) {
        return;
    }

    if (!$table.find('tbody tr.project-step-row:visible').length) {
        addProjectStepRow($form, $container);
    }
}

function addProjectStepRow($form, $container) {
    var $table = $form.find('#project_steps_table');
    var template = $form.find('#project_step_row_template').html();

    if (!$table.length || !template) {
        return;
    }

    var index = parseInt($table.attr('data-next-index') || 0, 10);
...
$(document).on('click', '.add_project_step_row', function() {
    var $form = $(this).closest('form#project_form');
    addProjectStepRow($form, $('#project_model'));
});

$(document).on('click', '.remove_project_step_row', function() {
    var $row = $(this).closest('tr.project-step-row');
    var $form = $(this).closest('form#project_form');

    if ($row.find('input[name$="[id]"]').val()) {
        $row.find('.project-step-delete-flag').val(1);
        $row.hide();
    } else {
        $row.remove();
    }

    initializeProjectStepRows($form, $('#project_model'));
});
...
$('#project_model').on('shown.bs.modal', function (e) {
    ...
    initializeProjectFormModal($(this));
});
...
$(document).on('click', '#project_model [data-dismiss="modal"], #project_model [data-bs-dismiss="modal"], #project_model .close', function(e) {
    e.preventDefault();
 ...
 */
function initializeProjectTaskModal($modal) {
    var $container = $modal && $modal.length ? $modal : $('.project_task_model');
    var $form = $container.find('form#project_task_form');

    if (!$form.length) {
        $form = $('form#project_task_form');
    }
...
$('.project_task_model').on('shown.bs.modal', function (e) {
    initializeProjectTaskModal($(this));
...
$(document).on('click', '.project_task_model [data-dismiss="modal"], .project_task_model [data-bs-dismiss="modal"], .project_task_model .close', function(e) {
    e.preventDefault();
...
function initialize_dropzone() {
    ...
    dropzoneInstance = $("div#fileupload").dropzone({
            url: projectModuleUrl('/project/post-media-dropzone-upload'),
            paramName: 'file',
            uploadMultiple: true,
 ...
 */
function initializeTimeLogModal($modal) {
    var $container = $modal && $modal.length ? $modal : $('form#time_log_form').closest('.modal');
    var $form = $container.find('form#time_log_form');

    if (!$form.length) {
        $form = $('form#time_log_form');
    }
...
$('#time_log_model').on('shown.bs.modal', function(e) {
    initializeTimeLogModal($(this));
});

$('#time_log_model').on('hidden.bs.modal', function() {
    $(this).html('');
});

$(document).on('click', '#time_log_model [data-dismiss="modal"], #time_log_model [data-bs-dismiss="modal"], #time_log_model .close, .view_modal form#time_log_form [data-dismiss="modal"], .view_modal form#time_log_form [data-bs-dismiss="modal"], .view_modal form#time_log_form .close', function(e) {
    e.preventDefault();
...
function initializeMyTaskDataTable() {
    ...
    if ((typeof my_task_datatable == 'undefined') && task_view == 'list_view') {
        my_task_datatable = $("#my_task_table").DataTable({
                ...
                serverSide: true,
                ajax:{
                    url: projectModuleUrl('/project/project-task'),
                    data: function(d) {
                        d.project_id = $('#project_id').val();
                        d.location_id = $('#task_location_filter').val();
...
$(document).on('change', "#project_id, #task_location_filter, #assigned_to_filter, #status_filter, #due_date_filter, #priority_filter", function(){
    initializeMyTaskDataTable();
});

$(document).on('change', '.my_task_view', function() {
    if (($(this).val() == 'kanban') && $('.custom-kanban-board').hasClass('hide')) {
        $('.my-task-table-wrap, .status_filter').addClass('hide');
        $('.custom-kanban-board').removeClass('hide');
        initializeTaskKanbanBoard();
    } else if ($(this).val() == 'list_view') {
        $('.custom-kanban-board').addClass('hide');
        $('.my-task-table-wrap, .status_filter').removeClass('hide');
        initializeMyTaskDataTable();
    }
});

// on change project filter get project
$(document).on('change', "#project_location_filter, #project_status_filter, #project_end_date_filter, #project_categories_filter", function(){
...
function getProjectList(url = '') {
    ...
    var data = {
            ...
            'location_id' : $('#project_location_filter').val(),
    ...
    if (url.length == 0) {
        url = projectModuleUrl('/project/project');
    }
...
$(document).on('change', '.task_view', function() {
    if (($(this).val() == 'kanban') && $('.custom-kanban-board').hasClass('hide')) {
        $('#project_task .project-task-table-wrap, #project_task .status_filter').addClass('hide');
        $('.custom-kanban-board').removeClass('hide');
        initializeTaskKanbanBoard();
    } else if ($(this).val() == 'list_view') {
        $('.custom-kanban-board').addClass('hide');
        $('#project_task .project-task-table-wrap, #project_task .status_filter').removeClass('hide');
        initializeProjectTaskDatatable();
    }
});
...
function getTaskListForKanban() {
   var data = {
        ...
        location_id : $('#task_location_filter').val(),
...
function updateProjectTaskStatusForKanban(data, el) {
    $.ajax({
        method: 'PUT',
        dataType: 'json',
        url: projectModuleUrl('/project/project-task/' + data.task_id + '/post-status'),
        data:data,
        success: function(result){
            if (result.success) {
...
function initializeProjectTaskDatatable() {
    ...
    if((typeof project_task_datatable == 'undefined') && task_view == 'list_view') {
        ...
        project_task_datatable.ajax.reload(function() {
            project_task_datatable.columns.adjust();
        }, false);
    } else if (task_view == 'kanban') {
        initializeTaskKanbanBoard();
...
function initializeTimeLogDatatable() {
    if(typeof time_logs_data_table == 'undefined') {
        time_logs_data_table = $('#time_logs_table').DataTable({
            ...
            serverSide: true,
            ajax:{
                url: projectModuleUrl('/project/project-task-time-logs'),
                data: function(d) {
                    d.project_id = $('#project_id').val();
                }
            },
...
function initializeActivities() {
    ...
    $.ajax({
        method:'GET',
        dataType: 'json',
        url: projectModuleUrl('/project/activities'),
        data: data,
        success: function(result){
            if (result.success) {
...
function initializeInvoiceDatatable() {
    if (typeof project_invoice_datatable == 'undefined') {
        project_invoice_datatable = $('#project_invoice_table').DataTable({
            ...
            serverSide: true,
            ajax:{
                url: projectModuleUrl('/project/invoice'),
                data: function(d) {
                    d.project_id = $('#project_id').val();
                }
            },
...
function updateProjectStatusForKanban(data, el) {
    $.ajax({
        method: 'PUT',
        dataType: 'json',
        url: projectModuleUrl('/project/project/'+ data.project_id + '/post-status'),
        data:data,
        success: function(result){
            if (result.success) {
                $(el).attr('data-parentid', data.status);
...
$(document).on('change', '#employee_timelog_report_location_id, #employee_timelog_report_user_id, #employee_timelog_report_project_id, #employee_timelog_report_daterange', function() {
...
function getEmployeeTimeLogReport() {
    var data = {
        ...
        location_id: $('#employee_timelog_report_location_id').val(),
    ...
    $.ajax({
        method: 'GET',
        dataType: 'json',
        url: projectModuleUrl('/project/project-employee-timelog-reports'),
        data: data,
        success: function(result) {
            if (result.success) {
...
$(document).on('change', '#project_timelog_report_location_id, #project_timelog_report_project_id, #project_timelog_report_daterange', function() {
...
function getProjectTimeLogReport() {
    ...
    var data = {
        ...
        location_id: $('#project_timelog_report_location_id').val(),
    ...
    $.ajax({
        method: 'GET',
        dataType: 'json',
        url: projectModuleUrl('/project/project-timelog-reports'),
        data:data,
        success:function(result) {
            if (result.success) {
    ...
    swal({
        ...
        if (confirmed) {
            $.ajax({
                method:'DELETE',
                dataType: 'json',
                url: projectModuleUrl('/project/project-task-comment/'+comment_id+'?task_id='+task_id),
                success: function(result){
                    if (result.success) {
                        toastr.success(result.msg);
